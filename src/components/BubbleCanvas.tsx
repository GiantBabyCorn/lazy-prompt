...
/* ------------------------------------------------------------------ */
/*  Constants (from bubbleConstants.ts)                                 */
/* ------------------------------------------------------------------ */

import {
  BUBBLE_SIZES as SIZES,
  REPULSION_STRENGTH,
  POS_SPRING,
  MORPH_SPRING,
  SCALE_REFERENCE,
...
/** Fraction of bubble radius used as hover buffer zone. */
const HOVER_MARGIN_FRACTION = 0.45;
...
interface BubbleCanvasProps {
  ...
  onSkipNavigate: (nodeIds: string[]) => void;
  onGoBack: () => void;
...
export function BubbleCanvas({
  ...
  void _t;
  const { t: _tPrompts } = useTranslation('prompts');
  void _tPrompts;
  ...
  const scaleFactor = Math.min(SCALE_MAX, Math.max(SCALE_MIN, scaleBasis / SCALE_REFERENCE));
  const S = {
  ...
  const goBackPos = useMemo(() => {
    ...
    return computeEdgePosition(cx, cy, entryDirection, dims.w, dims.h, S.goBack + 26);
  ...
  /* --- Hover margin: buffer zone so previews don't vanish while moving toward them --- */
  ...
  /** Check if cursor is within the parent bubble OR any of its preview children. */
  const isCursorInHoverZone = useCallback(() => {
    if (!hoveredNodeId) return false;
    const { x: mx, y: my } = mousePos.current;
    // Check parent child margin
    const pos = childPositions.find((cp) => cp.node.id === hoveredNodeId);
    if (pos) {
      const dist = Math.sqrt((mx - pos.cx) ** 2 + (my - pos.cy) ** 2);
      if (dist <= pos.radius + pos.radius * HOVER_MARGIN_FRACTION) return true;
    }
    // Check preview children — cursor on any preview keeps parent hovered
    for (const pp of previewPositions) {
      const dist = Math.sqrt((mx - pp.cx) ** 2 + (my - pp.cy) ** 2);
      if (dist <= pp.radius + pp.radius * HOVER_MARGIN_FRACTION) return true;
    }
    return false;
  }, [hoveredNodeId, childPositions, previewPositions]);

  const handleHoverEnd = useCallback(
    (nodeId: string) => {
      if (isCursorInHoverZone()) return;
  ...
  // Periodic check: clear hover when cursor truly leaves the zone
  useEffect(() => {
    if (!hoveredNodeId || animationPhase !== 'idle') return;
    const interval = setInterval(() => {
      if (!isCursorInHoverZone()) {
        setHoveredNodeId(null);
        setHoveredPreviewId(null);
      }
    }, 100);
    return () => clearInterval(interval);
  }, [hoveredNodeId, animationPhase, isCursorInHoverZone]);
  ...
  /** Skip-navigate: click a preview (grandchild) to jump 2 levels at once. */
  const handlePreviewClick = useCallback(
    (previewNode: BubbleNodeType) => {
      if (animationPhase !== 'idle' || !hoveredNodeId || !hoveredChild) return;

      // Set up exit morph for the hovered parent child
      const pos = basePositions.find((cp) => cp.node.id === hoveredNodeId);
      if (pos) {
        const dir = normalize(pos.cx - cx, pos.cy - cy);
        // Push direction for the child level; compute grandchild direction
        const grandchildren = hoveredChild.children ?? [];
        const gcIdx = grandchildren.findIndex((c) => c.id === previewNode.id);
        const gcAngle = grandchildren.length > 0
          ? (2 * Math.PI * gcIdx) / grandchildren.length - Math.PI / 2
          : 0;
        setDirectionStack((prev) => [
          ...prev,
          dir,
          { x: Math.cos(gcAngle), y: Math.sin(gcAngle) },
        ]);
        clickedPositionRef.current = { cx: pos.cx, cy: pos.cy, radius: pos.radius };
      }
      ...
      setHoveredNodeId(null);
      setHoveredPreviewId(null);
      onSkipNavigate([hoveredNodeId, previewNode.id]);
    },
    [animationPhase, hoveredNodeId, hoveredChild, basePositions, cx, cy, onSkipNavigate],
  );
  ...
  const _goBackLabelNode = animationPhase === 'idle' ? parentNode : (previousNode ?? parentNode);
  void _goBackLabelNode; // no longer used — go-back shows generic "← Go Back"
  ...
  return (
    ...
    >
      ...
      >
        ...
        {childPositions.map((child, i) => {
          ...
          return (
            <BubbleNodeSvg
              ...
              onMouseLeave={() => handleHoverEnd(child.node.id)}
              isEntering={isEntering || initialEntrance}
        ...
        <AnimatePresence>
          {previewPositions.map((preview, i) => {
            const isPreviewLeaf = !!preview.node.promptTemplateId && !preview.node.children?.length;
            return (
              <BubbleNodeSvg
                ...
                isLeaf={isPreviewLeaf}
                isHovered={hoveredPreviewId === preview.node.id}
                onClick={() => handlePreviewClick(preview.node)}
                onMouseEnter={() => setHoveredPreviewId(preview.node.id)}
